...
void Game::update() {
	...
	if(collision(0)) { //If there is a collision
...
void Game::restart()
{
	...
	killed = false;
	paused = false;
	active = Piece(rand() % 7);  //Create a new random piece
	
	active.x = COLS/2; //Set its x and y location to the starting points
	active.y = 0;
...
bool Game::canRotate(Piece activeP) {
	if(collision(-13)) {  //-13 because I just set the collision method to check for rotations if a value < 10 is being passed
		return false;
	}
	else
		return true;
...
*/
bool Game::collision(int dir) {
	//temporary values for the modified x and y
	int x, y;
	bool ret = false;
	if(dir < -10) {       //If the user is trying to rotate a piece
